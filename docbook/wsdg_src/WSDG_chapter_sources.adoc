...
does not contain trivial errors.
...
repository directory, there will be a git-hooks/ directory, with project's git hooks
...
you have to change the git hooks directory with `git config --local core.hooksPath git-hooks`
and make sure it is executable or it will not be run. With this command we have changed
the hooks directory: if we want to use the standard git hooks (the ones provided as
sample in the .git/hooks directory), we have to copy them into git-hooks/ of the
Wireshark tree.
+
